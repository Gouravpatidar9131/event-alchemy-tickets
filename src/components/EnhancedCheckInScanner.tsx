...
const EnhancedCheckInScanner = ({
  eventId
}: EnhancedCheckInScannerProps) => {
  ...
  } = useEventAttendanceQuery(eventId);

  // Real-time attendance updates
  useEffect(() => {
    const channel = supabase.channel(`attendance-${eventId}`).on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
      table: 'attendance',
  ...
  return <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            {event?.nft_enabled && <Badge variant="secondary" className="ml-2">
                NFT Enabled
              </Badge>}
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input placeholder="e.g., Main Entrance, VIP Gate" value={checkInLocation} onChange={e => setCheckInLocation(e.target.value)} />
          </div>

          {/* Scanner Section */}
          <div className="space-y-4">
            <div className="flex gap-2">
              {!isScanning ? <Button onClick={startScanning} className="flex items-center gap-2">
                  <Camera className="h-4 w-4" />
                  Start QR Scanner
                </Button> : <Button onClick={stopScanning} variant="outline" className="flex items-center gap-2">
                  <XCircle className="h-4 w-4" />
                  Stop Scanner
                </Button>}
            </div>

            {isScanning && <div className="border rounded-lg p-4 bg-gray-50">
                <QrScanner onScan={handleQRScan} onError={handleQRError} styles={{
              container: {
                width: '100%',
                maxWidth: '400px',
                height: '300px'
          ...
          <div className="space-y-4 pt-4 border-t">
            ...
            <div className="flex gap-2">
              <Input placeholder="Paste QR code data or enter ticket ID" value={manualInput} onChange={e => setManualInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleManualCheckIn()} />
              <Button onClick={handleManualCheckIn} disabled={isProcessing || !manualInput.trim()} className="flex items-center gap-2">
          ...
          {/* Result Display */}
          {scanResult && <Card className={`border-2 ${scanResult.success ? 'border-green-500 bg-green-50' : 'border-red-500 bg-red-50'}`}>
              <CardContent className="pt-4">
                <div className="flex items-center gap-2">
                  {scanResult.success ? <CheckCircle className="h-5 w-5 text-green-600" /> : <XCircle className="h-5 w-5 text-red-600" />}
                  <span className={`font-medium ${scanResult.success ? 'text-green-800' : 'text-red-800'}`}>
                    {scanResult.message}
                  </span>
                </div>
                {scanResult.attendanceData && <div className="mt-3 space-y-2">
                    <div className="text-sm text-gray-600">
                      <p>Checked in at: {new Date(scanResult.attendanceData.checked_in_at).toLocaleString()}</p>
                      {scanResult.attendanceData.check_in_location && <p>Location: {scanResult.attendanceData.check_in_location}</p>}
                    </div>
                    {event?.nft_enabled && <NFTAttendanceBadge attendance={scanResult.attendanceData} eventNftEnabled={event?.nft_enabled} />}
                  </div>}
              </CardContent>
            </Card>}
        </CardContent>
      ...
      {/* Recent Check-ins */}
      {attendance && attendance.length > 0 && <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
          ...
          <CardContent>
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {attendance.slice(0, 10).map(record => <div key={record.id} className="space-y-2">
                  <div className="flex items-center justify-between p-2 rounded bg-gray-950">
                    <div className="flex items-center gap-2">
                      <User className="h-4 w-4" />
                      <span className="font-mono text-sm">{record.ticket_id.slice(0, 8)}...</span>
                      {record.check_in_location && <Badge variant="secondary" className="text-xs">
                  ...
                  {event?.nft_enabled && <NFTAttendanceBadge attendance={record} eventNftEnabled={event?.nft_enabled} />}
                </div>)}
            </div>
          </CardContent>
        </Card>}
