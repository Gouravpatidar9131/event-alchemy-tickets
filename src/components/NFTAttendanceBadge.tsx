...
const NFTAttendanceBadge = ({
  attendance,
  eventNftEnabled = false
}: NFTAttendanceBadgeProps) => {
  ...
  return <Card className="border-l-4 border-l-purple-500">
      ...
      <CardContent className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          {attendance.nft_minted_at && <span className="text-xs text-muted-foreground">
              {new Date(attendance.nft_minted_at).toLocaleDateString()}
            </span>}
        </div>

        {attendance.nft_status === 'minted' && attendance.nft_mint_address && <div className="space-y-2">
            <div className="p-2 rounded text-xs bg-gray-950">
              <p className="font-mono break-all">{attendance.nft_mint_address}</p>
            </div>
            <div className="flex gap-2">
              {attendance.nft_metadata_uri && <Button size="sm" variant="outline" className="flex-1 text-xs" onClick={() => window.open(attendance.nft_metadata_uri!, '_blank')}>
                  <ExternalLink className="h-3 w-3 mr-1" />
                  Metadata
                </Button>}
              {getOpenSeaUrl() && <Button size="sm" variant="outline" className="flex-1 text-xs" onClick={() => window.open(getOpenSeaUrl()!, '_blank')}>
                  <ExternalLink className="h-3 w-3 mr-1" />
                  OpenSea
                </Button>}
            </div>
        ...
        {attendance.nft_status === 'pending' && <div className="space-y-2">
            ...
            <Button size="sm" onClick={handleMintNFT} disabled={mintAttendanceNFTMutation.isPending} className="w-full text-xs">
        ...
        {attendance.nft_status === 'failed' && <Button size="sm" variant="outline" onClick={handleMintNFT} disabled={mintAttendanceNFTMutation.isPending} className="w-full text-xs">
            <Sparkles className="h-3 w-3 mr-1" />
            Retry Minting
          </Button>}
      </CardContent>
